...
import com.hescha.computerstore.service.OrderItemService;
...
@RequestMapping(OrderItemController.CURRENT_ADDRESS)
public class OrderItemController {
    public static final String API_PATH = "orderitem";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new OrderItem());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute OrderItem entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                OrderItem createdEntity = service.create(entity);
