...
import com.hescha.computerstore.service.ProcessorsService;
...
@RequestMapping(ProcessorsController.CURRENT_ADDRESS)
public class ProcessorsController {
    public static final String API_PATH = "processors";
    ...
    private final ProcessorsService service;
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Processors());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Processors entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Processors createdEntity = service.create(entity);
