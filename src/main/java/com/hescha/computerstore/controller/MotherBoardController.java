...
import com.hescha.computerstore.service.MotherBoardService;
...
@RequestMapping(MotherBoardController.CURRENT_ADDRESS)
public class MotherBoardController {
    public static final String API_PATH = "motherboard";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new MotherBoard());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute MotherBoard entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                MotherBoard createdEntity = service.create(entity);
