...
import com.hescha.computerstore.service.PeriferiaService;
...
@RequestMapping(PeriferiaController.CURRENT_ADDRESS)
public class PeriferiaController {
    public static final String API_PATH = "periferia";
    ...
    private final PeriferiaService service;
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Periferia());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Periferia entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Periferia createdEntity = service.create(entity);
