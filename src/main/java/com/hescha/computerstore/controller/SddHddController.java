...
import com.hescha.computerstore.service.SddHddService;
...
@RequestMapping(SddHddController.CURRENT_ADDRESS)
public class SddHddController {
    public static final String API_PATH = "sddhdd";
    ...
    private final SddHddService service;
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new SddHdd());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute SddHdd entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                SddHdd createdEntity = service.create(entity);
