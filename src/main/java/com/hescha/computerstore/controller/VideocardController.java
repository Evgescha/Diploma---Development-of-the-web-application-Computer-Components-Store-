...
import com.hescha.computerstore.service.VideocardService;
...
@RequestMapping(VideocardController.CURRENT_ADDRESS)
public class VideocardController {
    public static final String API_PATH = "videocard";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Videocard());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Videocard entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Videocard createdEntity = service.create(entity);
