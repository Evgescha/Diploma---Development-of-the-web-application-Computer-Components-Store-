...
import com.hescha.computerstore.service.RamService;
...
@RequestMapping(RamController.CURRENT_ADDRESS)
public class RamController {
    public static final String API_PATH = "ram";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Ram());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Ram entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Ram createdEntity = service.create(entity);
